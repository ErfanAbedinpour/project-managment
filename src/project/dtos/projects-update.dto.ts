import { IsBoolean, IsEnum, IsNotEmpty, IsOptional, IsString, MaxLength, MinLength, Validate, ValidationArguments, ValidatorConstraint, ValidatorConstraintInterface } from "class-validator";
...
class AtOneLeastRequired implements ValidatorConstraintInterface {
    validate(_: unknown, validationArguments?: ValidationArguments): Promise<boolean> | boolean {
        ...
        return !!(myDto.name || myDto.description || myDto.isPublic || myDto.status)
    }

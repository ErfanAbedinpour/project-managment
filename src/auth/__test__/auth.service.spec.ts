...
describe('Auth Service ', () => {
    ...
    describe("register", () => {
        ...
        it("Should be throw BadRequest because email is taken", async () => {
            ...
            await expect(authService.register(
                { email: user.email, display_name: user.display_name, password: user.password, username: user.username }
            )).rejects.toThrow('email already taken.')
            expect(prisma.user.findFirst).toBeDefined()
            expect(prisma.user.findFirst).toHaveBeenCalledWith({ where: { email: user.email } })
        })
        ...
        it("Should be thorw Bad request for username is invalid", async () => {
            ...
            expect(prisma.user.findFirst).toHaveBeenCalledWith({ where: { email: user.email } })
            expect(prisma.user.findFirst).toHaveBeenLastCalledWith({ where: { username: user.username } })
        })
        ...
        it("Should be user created succesfully", async () => {
            ...
            hash.hash.mockImplementationOnce((password: string) => Promise.resolve(`$${password}`))
    ...
    describe("login", () => {
        ...
        it("Should be throw NotFound for user does not exsist", async () => {
            ...
            await expect(authService.login(loginUser))
                .rejects.
                toThrow("user does not found")
        })
        ...
        it('Should be throw BadRequest for wrong password', async () => {
            ...
            await expect(authService.login(loginUser)).rejects.toThrow("identify or password are incorrect")

            expect(hash.compare).toHaveBeenCalledWith(loginUser.password, user.password)
        ...
        it('Should be return accessToken and refreshToken', async () => {
            ...
            const tokens = { accessToken: String(Math.random() * 1000), refreshToken: String(Math.random() * 1000) }
            userToken.getKeys.mockResolvedValueOnce(tokens)

            await expect(authService.login(loginUser)).resolves.toEqual(tokens)
            expect(hash.compare).toHaveBeenCalledWith(loginUser.password, user.password)
            expect(prisma.user.findFirst).toHaveBeenCalledWith({ where: { OR: [{ email: loginUser.identify }, { username: loginUser.identify }] } })
    ...
    describe("RefreshToken", () => {
        ...
        it("should be throw if token invalid", async () => {
            ...
            await expect(authService.refreshToken(token)).rejects.toThrow(UnauthorizedException)
            expect(userToken.isValid).toHaveBeenCalledWith({ userId: user.id, token })
            expect(userToken.isValid).not.toHaveBeenCalledWith({ userId: 3, token: "asdf" })
        ...
        it('Should be generate new tokens', async () => {
            ...
            expect(prisma.user.findFirst).toHaveBeenCalledWith({ where: { id: user.id } });

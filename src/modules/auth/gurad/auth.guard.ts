...
export class AuthGurad implements CanActivate {
  constructor(
    ...
    private AccessTokenGurad: AccessTokenGurad,
  ) {}

  async canActivate(context: ExecutionContext): Promise<boolean> {
    const meta = this.reflector.getAllAndOverride<AuthStrategy[]>(
      AUTH_META_KEY,
      [context.getHandler(), context.getClass()],
    ) ?? [AuthStrategy.Bearer];

    const isUserSetHeader =
    ...
    /*
         ...
         */
    if (!isUserSetHeader && meta.includes(AuthStrategy.None)) return true;
